...
export default async (req) => {
  ...
  if (req.method === 'PUT') {
    ...
    // Notify captain + co-captains on any REAL change (first-time in/out, or a
    // flip). No email for no-op re-saves. Awaited but never fatal.
    if (prev !== status) {
      try { await notifyCaptainsOfChange({ team, player, actingEmail: ctx.session?.email || player.email, match, status, reason: mine.reason }); }
