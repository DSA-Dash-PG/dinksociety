...
/**
 * Lightweight predicate: is the caller an ACTIVE organizer who owns this event?
 * Used by authScoreAccess (lib/ladder-scorer.js) to let an organizer run their
 * own ladder's roster/scoring through the existing admin-ladder-* endpoints
 * without loosening access for anyone else. Returns false fast when there's no
 * player session (e.g. a courtside scorer-token request) so it adds ~one blob read.
 */
export async function isOrganizerOwner(req, eventId) {
